  ...
  return (
    <div style={{ display:'flex', gap:10, padding:'4px 0', flexDirection: isAxon ? 'row' : 'row-reverse', animation:'fadeUp .25s ease both' }}>
      <div style={{ width:28, height:28, borderRadius:7, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, marginTop:3,
        ...(isAxon ? { background:'linear-gradient(135deg,#5b8dee,#8bb4ff)', color:'#fff', fontFamily:'Syne,sans-serif', boxShadow:'0 0 10px var(--glow)' }
...
export default function Page() {
  ...
  const newChat = () => { setSid(null); setMsgs([]); setSidebarOpen(false); };
    ...
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', position:'relative' }}>
      ...
      <main style={{ flex:1, display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden', marginLeft:0 }}>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'16px 14px 8px', display:'flex', flexDirection:'column', gap:3 }}>
          ...
          {msgs.length === 0 && !busy && (
            <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', gap:16, padding:'20px 10px', animation:'fadeUp .5s ease both' }}>
              <div style={{ width:60, height:60, background:'linear-gradient(135deg,#5b8dee,#8bb4ff)', borderRadius:16, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 0 30px rgba(91,141,238,.28)' }}>
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.9"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
              </div>
              <div>
                <h1 style={{ fontFamily:'Syne,sans-serif', fontSize:24, fontWeight:800, letterSpacing:'-0.5px', marginBottom:8 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, width:'100%', maxWidth:440 }}>
                {SUGGESTIONS.map((s,i) => (
                  <div key={i} onClick={() => send(s.prompt)}
                    style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'11px 13px', cursor:'pointer', textAlign:'left' }}>
          ...
          {busy && (
            <div style={{ display:'flex', gap:10, padding:'4px 0' }}>
              <div style={{ width:28, height:28, borderRadius:7, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, marginTop:3, background:'linear-gradient(135deg,#5b8dee,#8bb4ff)', color:'#fff', fontFamily:'Syne,sans-serif' }}>A</div>
              <div style={{ padding:'12px 14px', borderRadius:13, background:'var(--surface)', border:'1px solid var(--border)', borderTopLeftRadius:3, display:'flex', gap:5, alignItems:'center' }}>
                {[0,1,2].map(i => <div key={i} style={{ width:6, height:6, background:'var(--accent)', borderRadius:'50%', animation:`dot 1.2s ease-in-out ${i*.2}s infinite` }}/>)}
              </div>
            </div>
          )}
        ...
        <div style={{ padding:'10px 14px 16px', borderTop:'1px solid var(--border)', flexShrink:0 }}>
          <div style={{ background:'var(--surface)', border:'1px solid var(--border2)', borderRadius:12, display:'flex', alignItems:'flex-end', padding:'3px 3px 3px 13px' }}>
            ...
            <button onClick={() => send()} disabled={busy||!input.trim()}
              style={{ width:36, height:36, borderRadius:9, background: busy||!input.trim() ? 'var(--border2)' : 'var(--accent)', border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor: busy||!input.trim() ? 'not-allowed':'pointer', transition:'all .15s', flexShrink:0, margin:2 }}>
          ...
          <div style={{ textAlign:'center', paddingTop:6, fontSize:11, color:'var(--muted)' }}>
            Powered by <span style={{ color:'var(--accent)' }}>Cerebras</span>
          </div>
        </div>
      </main>
    </div>
